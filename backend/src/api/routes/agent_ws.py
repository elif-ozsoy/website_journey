...
async def websocket_run(websocket: WebSocket):
    ...
    try:
        ...
        # Strip persona prefix injected by the frontend before using as the stored title
        import re as _re
        original_task = _re.sub(r'^\[Persona:[^\]]*\]\s*\n+', '', task).strip()
            ...
            try:
                ...
                try:
                    ...
                    if not db.get(SiteModel, site_id):
                        ...
                        # Strip screenshot blobs before storing in the journey record —
                        # they are large (500KB–1MB each) and already saved separately
                        # in the screenshots table by persist_browseruse_screenshots.
                        steps_lean = [
                            {k: v for k, v in s.items() if k != "screenshot_base64"}
                ...
                print(f"[WS] ERROR: journey persistence failed for site_id={site_id}: {type(exc).__name__}: {exc}", flush=True, file=sys.stderr)
                traceback.print_exc()
                try:
                    await websocket.send_json({"type": "warning", "message": f"Journey saved to DB failed: {type(exc).__name__}: {exc}"})
                except Exception:
                    pass
