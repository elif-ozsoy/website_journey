...
def list_tasks(site_id: str, db: Session = Depends(get_db)):
    return svc.list_tasks(db, site_id)
...
def create_task(site_id: str, body: TaskCreate, db: Session = Depends(get_db)):
    return svc.create_task(db, site_id, body.title, body.description)


@router.put("/tasks/{task_id}", response_model=TaskResponse)
def update_task(task_id: int, body: TaskUpdate, db: Session = Depends(get_db)):
    task = svc.update_task(db, task_id, body.title, body.description)
    if task is None:
