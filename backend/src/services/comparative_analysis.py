...
- For diagrams: select 0–3 diagrams per point — only those where the evidence is most directly visible. If no diagram genuinely shows this issue, use an empty array. Never pad with diagrams just for completeness. Avoid selecting multiple diagrams that show the same kind of evidence (e.g. "compare" and "insights" both show step counts — pick the better one, not both). Available views and what they show:
  ...
  * "horizon" — per-journey activity density strip chart; x-axis = relative journey time (0–100%), colour darkness = action intensity at that moment (clicks, scrolls, inputs, navigation). Each journey is one horizontal strip. For horizon you MUST include a "highlight" object with "side": "ai" when the evidence is about agent timing/rhythm, "side": "human" when about human timing, "side": "both" when comparing both. Best for: comparing WHEN in the journey activity is concentrated, front/back-loaded task patterns, agents that front-load navigation while humans explore gradually, temporal differences in exploration rhythm between AI and human sessions, unusually long or short phases in certain journeys.
...
- For diagrams: select at most 2 per point — one for agent evidence, one for human evidence if genuinely different. For "agent_gap" type points, prefer "sankey" with side "ai" (to highlight which agent journeys failed or diverged) or "horizon" with side "ai" (to show when in the journey agents behave differently). For "human_issue" type points, prefer "sankey" with side "human" or "horizon" with side "human". For "ux_issue" type, prefer "sankey" with side "both" or "heatmap". Use "policy" when you want to show whether the issue is correctable by injecting human context into the agent. Never pad — if only 1 diagram genuinely shows the evidence, use 1. Empty array is valid.
"""
